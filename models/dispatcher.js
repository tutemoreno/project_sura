...
export default class Dispatcher {
  constructor(
    ...
    this.currentCalls = 0; // contador de llamadas concurrentes
  }
  ...
  dispatchCall() {
    return new Promise((resolve) => {
      if (!this.callsQueue.length) return resolve({ message: 'no more calls' });
      if (!(this.currentCalls < maxCalls))
  ...
  startCall(call) {
    return new Promise((resolve) => setTimeout(() => resolve(), call.duration));
  }
}
